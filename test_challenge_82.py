...
# OTHER_LANG_COMMAND = "/home/user/Dev/Cpp/c80_cpp"
...
def print_extra_stats(
    ...
    print(
        f" - Average string length: {yellow}"
        f"{sum(len(x[0]) for x in test_inp) // num_cases:_}{reset}"
    )
    print(
        f" - Max string length: {yellow}"
        f"{max(len(x[0]) for x in test_inp)}{reset}"
    )
...
def print_begin(
    ...
    *,
    ...
    if (
        ...
        and (lang is Lang.OTHER and not SOLUTION_FILE_NAME.endswith(".py") or lang is Lang.PYTHON)
    ):
...
if __name__ == "__main__":
    ...
    lang = Lang.OTHER if OTHER_LANG_COMMAND else Lang.PYTHON
